...
/* initGraph: initializes the given graph
...
void initGraph(Graph *G)
{
    ...
    { // iterates through every possible index
        G->adjacencyCount[i] = 0;
        G->vertices[i][0] = '\0';
        for (int j = 0; j < MAX_VERTICES; j++)
        {                                     // iterates through every possible adjacent vertex
            G->adjacencyList[i][j][0] = '\0'; // this clears each adjacency string
        }
    }
}

/* vertexExists: checks if a vertex already exist in the graph
   @param G          - holds the values of the graph
...
int vertexExists(Graph *G, char *vertexName)
{
    ...
    for (int i = 0; i < G->vertexCount; i++)
    {
        ...
        { // cheks if the vertex is already in the graph
            returnVal = 1;
            break;
        }
    }
...
/* vertexExists: adds a vertex that is not already in the graph
   @param G          - holds the values of the graph
   @param vertexName - holds the name of the vertex to be added in the graph
*/
void addVertex(Graph *G, char *vertexName)
{
    if (vertexExists(G, vertexName) == 0 && G->vertexCount < MAX_VERTICES)
    { // checks if the vertex does not exist in the graph and if the graph has reached its maximum number of vertices
        strcpy(G->vertices[G->vertexCount], vertexName);
        G->vertexCount++;
    }
}

/* adjacencyExist: checks if the nextVertex is already in the adjacency list of the primary vertex
   @param G            - holds the values of the graph
...
int adjacencyExist(Graph *G, char *nextVertex, int u)
{
    ...
    {
        if (strcmp(G->adjacencyList[u][i], nextVertex) == 0)
        { // itarates throgh the adjacency list
            returnVal = 1;
        }
    }
    return returnVal;
...
/* addAdjacency: adds the adjacencies of the vertices to a list
   @param G - holds the values of the graph
...
void addAdjacency(Graph *G, char *firstVertex, char *nextVertex)
{
    ...
    if (u == -1)
    { // checks if the index of the primary vertex is valid
        return;
    }
...
/* readInput: reads the name of the input file given by the user
...
int readInput(char *fileName, Graph *G, Graph *sortedG)
{
    ...
    else
    {
        ...
        { // iterate trough the file based on the number of vertices
            ...
            while (fscanf(file, "%s", firstWord) == 1 && strcmp(firstWord, "-1") != 0)
            {                                        // iterate though the line adding each vertex up until reaching -1
...
/* sortGraph: sorts the graph's vertices and the corresponding adjacency lists alphabetically
   @param G - the graph data structure to sort
*/
void sortGraph(Graph *G)
{
    ...
    for (int i = 0; i < G->vertexCount - 1; i++)
    {
        ...
        { // iterates through all the vertices
            if (strcmp(G->vertices[i], G->vertices[j]) > 0)
            { // sorts the vertices alphabetically
...
/* getVertexIndex: used to get the indexes of the vertices
   @param G          - holds the values of the graph
...
int getVertexIndex(Graph *G, char *vertexName)
{
    int vertexIndex = -1;
    for (int i = 0; i < G->vertexCount; i++)
    { // iterates through the vertices and gets their index
        if (strcmp(G->vertices[i], vertexName) == 0)
        {
            vertexIndex = i;
            break;
...
/* BFSTraversal: used to get the indexes of the vertices
   @param G           - holds the values of the graph
...
void BFSTraversal(Graph *G, char *startVertex, int *visitOrder, int *count)
{
    ...
    while (front < rear)
    {
        ...
        nextVertexCount = 0;
        for (int i = 0; i < G->adjacencyCount[current]; i++)
        { // iterate through adjacency list to find nextVertices
            adjacencyIndex = getVertexIndex(G, G->adjacencyList[current][i]);
            if (!visitedVertices[adjacencyIndex])
        ...
        for (int i = 0; i < nextVertexCount - 1; i++)
        { // sorts the nextVertices by vertex index(lowest ID first)
            for (int j = i + 1; j < nextVertexCount; j++)
            {
                if (nextVertices[i] > nextVertices[j])
                { // sorts the vertex indexes by ascending order
                    temp = nextVertices[i];
        ...
        for (int i = 0; i < nextVertexCount; i++)
        { // enqueue the nextVertices
...
/* DFSTraversal: implementation of a depth-first-traversal algorithim
   @param G                      - the graph data structure
   @param startVertex            - the vertex id of which to begin the traversal from. It is assumed that the start vertex exists
   @param traversalStringOutput  - a string to where the result of the traversal will be outut to
*/
