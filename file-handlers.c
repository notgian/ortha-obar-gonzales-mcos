...
/* outputSets: prints the  set of vertices V(G) and set of edges E(G) on a graph-SET.txt
...
void outputSets(char graphName, char *outputFileName, Graph *sortedG)
{
    ...
    for (int i = 0; i < sortedG->vertexCount; i++)
    {
        ...
        for (int j = 0; j < sortedG->adjacencyCount[i]; j++)
        {
            ...
            if (strcmp(u, v) < 0)
            { // avoids duplicates, Diana-Hal == Hal-Diana
                if (!first)
                {
                    fprintf(file, ",");
                }
...
/* outputDegress: prints the list of vertex IDs with the corresponding degree for each vertex
   @param fileName - holds the name of the input file
   @param sortedG  - holds the sorted values of the graph
*/
...
/* outputAdjacencyList: prints the adjacency List representation of the graph
   @param fileName - holds the name of the input file
...
void outputAdjacencyList(char *outputFileName, Graph *G)
{
    ...
    { // iterates through the primary vertices
        fprintf(outputFile, "%s", G->vertices[i]);
        for (int j = 0; j < G->adjacencyCount[i]; j++)
        { // iterates through the adjacency vertices
...
/* outputAdjacencyMatrix prints the adjacency matrix representation of the graph.
   @param fileName - holds the name of the input file
   @param G        - holds the values of the graph
*/
void outputAdjacencyMatrix(char *outputFileName, Graph *G)
{
    ...
    for (int i = 0; i < numVertices; i++)
    { // prints a row containing a primary vertex
        ...
        for (int j = 0; j < numVertices; j++)
        {
            ...
            {
                if (strcmp(G->adjacencyList[i][k], G->vertices[j]) == 0)
                { // checks if there is an adjacency
                    connected = 1;
                    break;
                }
            }
...
/* outputBFS: prints out the BFS textfile
   @param G           - holds the values of the graph
   @param fileName    - holds the name of the input file
